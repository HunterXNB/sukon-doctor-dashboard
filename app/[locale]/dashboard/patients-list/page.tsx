...
import { columns } from "@/components/patients-list/columns";
import PatientsListTable from "@/components/patients-list/PatientsListTable";
...
async function PatientListPage({ searchParams }: PatientListPageProps) {
  const t = await getTranslations("dashboardHome.topBar");
  const tPatientsList = await getTranslations("patientsList");
    ...
    <div className="flex flex-col @container">
      <div className="flex-1 gap-6 flex flex-col">
        ...
        <Suspense fallback={<LoadingStyledDataTable columns={columns} />}>
          <PatientsListTable searchParams={urlSearchParams} />
        </Suspense>
      </div>
    </div>
  );
